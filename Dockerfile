...
RUN apt install python3-pip -y && apt-get install -y nodejs && npm install -g yarn

# Install Yarn components
# RUN mkdir -p ${APP_ROOT}/accessibility/app/media/packages
WORKDIR ${APP_ROOT}/accessibility
RUN yarn install
...
COPY /accessibility/docker/entrypoint.sh ${WEB_ROOT}/entrypoint.sh
